...
def generate(
    ...
    frame_dir: Optional[str] = typer.Option(None, "--frames", "-f", help="Device frames directory"),
    ...
    try:
        ...
        if output_dir:
            ...
            console.print(f"📁 Overriding output directory: {output_dir}", style="yellow")
        else:
            console.print(f"📁 Using YAML output directory: {project_config.project.output_dir}", style="blue")
        ...
        try:
            ...
            for i, screenshot_def in enumerate(project_config.screenshots):
                if i < len(result_paths):
                    ...
                    results.append((screenshot_def.name, None, False, "Generation failed"))
        except Exception as e:
...
def list_frames(
    ...
    # Create table
    table = Table(title="Available Device Frames", show_header=True, header_style="bold magenta")
    table.add_column("Frame Name", style="cyan")
    table.add_column("Size", style="green")
    
    for frame_name in frames:
        size = generator.device_frame_renderer.get_frame_size(frame_name)
        size_str = f"{size[0]}×{size[1]}" if size else "Unknown"
...
def create_config(
    ...
    name: str = typer.Option("My Screenshot Project", "--name", "-n", help="Project name"),
) -> None:
...
def _show_results(results, output_dir: str) -> None:
    ...
    for name, path, success, error in results:
        if success:
            ...
            status = "❌ Failed"
            output_path = error[:50] + "..." if error and len(error) > 50 else (error or "")
