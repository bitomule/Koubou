...
from .config import ScreenshotConfig, ProjectConfig, BackgroundConfig, TextOverlay
from .exceptions import RenderError, DeviceFrameError, BackgroundRenderError, TextRenderError
...
class ScreenshotGenerator:
    ...
    def __init__(self, frame_directory: Optional[str] = None):
        ...
        """
        self.frame_directory = Path(frame_directory) if frame_directory else self._get_bundled_frames_path()
    ...
    def _load_frame_metadata(self) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to load frame metadata: {e}")
            self.frame_metadata = {}
            self.size_metadata = {}
    ...
    def generate_screenshot(self, config: ScreenshotConfig) -> Path:
        ...
        try:
            ...
            if config.image_frame and config.device_frame:
                ...
                positioned_image = self._apply_asset_frame(positioned_image, canvas, config)
            ...
            logger.error(f"❌ Generation failed for {config.name}: {e}")
            raise RenderError(f"Failed to generate screenshot '{config.name}': {e}") from e
    ...
    def _load_source_image(self, image_path: str) -> Image.Image:
        ...
        try:
            ...
            raise RenderError(f"Failed to load source image '{image_path}': {e}") from e
    
    def _position_source_image(self, source_image: Image.Image, canvas: Image.Image, config: ScreenshotConfig) -> Image.Image:
        ...
        logger.info(f"📏 Scaling image: {original_width}×{original_height} → {scaled_width}×{scaled_height} (scale: {scale_factor})")
        
        # Resize the source image
        if scale_factor != 1.0:
            source_image = source_image.resize((scaled_width, scaled_height), Image.Resampling.LANCZOS)
        ...
        logger.info(f"📐 Positioning asset: center at {position} → ({center_x}, {center_y}), top-left at ({x}, {y})")
    ...
    def _apply_device_frame_overlay(self, canvas: Image.Image, device_frame_name: str) -> Image.Image:
        ...
        try:
            ...
            if frame_image.size == canvas.size:
                ...
                logger.warning(f"Canvas size {canvas.size} doesn't match frame size {frame_image.size}")
                # For now, resize canvas to match frame
                resized_canvas = canvas.resize(frame_image.size, Image.Resampling.LANCZOS)
                return Image.alpha_composite(resized_canvas, frame_image)
                
        except Exception as e:
            logger.error(f"Failed to apply device frame overlay: {e}")
            return canvas  # Return original canvas if frame fails
    
    def _apply_asset_frame(self, positioned_image: Image.Image, canvas: Image.Image, config: ScreenshotConfig) -> Image.Image:
        """Apply device frame to individual asset with proper screen masking."""
        try:
            # Load and scale device frame image first
            frame_image = self.device_frame_renderer._load_frame_image(config.device_frame)
            ...
            scaled_frame = frame_image.resize((scaled_frame_width, scaled_frame_height), Image.Resampling.LANCZOS)
            
            logger.info(f"📱 Scaled frame: {original_frame_size} → {scaled_frame.size} (scale: {asset_scale})")
            
            # Generate screen mask from the already-scaled frame (preserves precise boundaries)
            screen_mask = self.device_frame_renderer.generate_screen_mask_from_image(scaled_frame)
            ...
            logger.info(f"📱 Positioning frame: center at ({center_x}, {center_y}), top-left at ({frame_x}, {frame_y})")
            ...
            logger.info(f"📱 Applying screen mask with precise boundaries")
            ...
            if frame_x >= 0 and frame_y >= 0 and frame_x + scaled_frame_width <= canvas.width and frame_y + scaled_frame_height <= canvas.height:
                ...
                logger.info(f"📱 Applied device frame overlay successfully")
                
            else:
                logger.warning(f"Frame would extend beyond canvas bounds - skipping frame overlay")
    ...
    def generate_project(self, project_config: ProjectConfig, config_dir: Optional[Path] = None) -> List[Path]:
        ...
        for i, screenshot_def in enumerate(project_config.screenshots, 1):
            ...
            try:
                ...
                temp_config = self._convert_to_screenshot_config(screenshot_def, device_frame, default_background, project_config.project.output_dir, config_dir)
                if temp_config:
                    output_path = self.generate_screenshot(temp_config)
                    results.append(output_path)
                else:
                    logger.warning(f"Skipping {screenshot_def.name}: no source image found")
            except Exception as e:
                logger.error(f"Failed to generate {screenshot_def.name}: {e}")
                # Continue with next screenshot instead of failing entire project
                continue
        
        logger.info(f"🎉 Project complete! Generated {len(results)}/{len(project_config.screenshots)} screenshots")
        return results
    
    def _resolve_output_path(self, output_dir: str, screenshot_name: str, config_dir: Optional[Path] = None) -> Path:
    ...
    def _convert_to_screenshot_config(self, screenshot_def, device_frame, default_background, output_dir, config_dir=None):
        ...
        for item in screenshot_def.content:
            if item.type == "image":
                ...
                if asset_path.startswith('../'):
                    source_image_path = str(Path('/Users/davidcollado/Projects').resolve() / asset_path[3:])
                else:
                ...
                logger.info(f"📏 Image: scale={image_scale * 100:.0f}%, position={image_position}, frame={getattr(item, 'frame', False)}")
                break  # Use first image found
        
        # Skip if no source image found
        if not source_image_path or not Path(source_image_path).exists():
            logger.warning(f"Source image not found for {screenshot_def.name}: {source_image_path}")
            return None
        ...
        logger.info(f"📐 Original: {original_width}×{original_height} → Scaled: {scaled_width}×{scaled_height}")
        ...
        if device_frame:
            ...
            if frame_size:
                ...
                logger.info(f"📐 Canvas: {canvas_width}×{canvas_height} (frame-based sizing)")
            else:
                logger.warning(f"Could not get frame size for {device_frame}, using content-based sizing")
                canvas_width = max(scaled_width + 400, 800)  # Minimum width for text, extra space for text
                canvas_height = scaled_height + 800  # Extra space for text above/below
                logger.info(f"📐 Canvas: {canvas_width}×{canvas_height} (content-based fallback)")
        else:
            # No frame: canvas = scaled image + padding for text
            canvas_width = max(scaled_width + 400, 800)  # Minimum width for text, extra space for text
            canvas_height = scaled_height + 800  # Extra space for text above/below  
            logger.info(f"📐 Canvas: {canvas_width}×{canvas_height} (content-based sizing)")
        ...
        for item in screenshot_def.content:
            if item.type == "text":
                ...
                if item.content:
                    position = self._convert_position(item.position, (canvas_width, canvas_height))
                    text_overlay = TextOverlay(
                        ...
                        max_width=getattr(item, 'maxWidth', None),  # User controls maxWidth, default None means no limit
                        max_lines=getattr(item, 'maxLines', None)  # None means unlimited lines with wrapping
                    )
        ...
        config = ScreenshotConfig(
            ...
            output_path=str(self._resolve_output_path(output_dir, screenshot_def.name, config_dir))
        )
