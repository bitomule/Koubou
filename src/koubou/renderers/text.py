...
class TextRenderer:
    ...
    def render(self, text_config: TextOverlay, canvas: Image.Image) -> None:
        ...
        try:
                # Load font
            font = self._get_font(text_config.font_family, text_config.font_size, text_config.font_weight)
            ...
            text_lines = self._prepare_text(text_config.content, font, text_config.max_width, text_config.max_lines, canvas.width)
            ...
            for i, line in enumerate(text_lines):
                ...
                line_x = self._calculate_line_x(anchor_x, line, font, text_config.alignment, text_block_width)
            ...
            raise TextRenderError(f"Failed to render text '{text_config.content[:50]}...': {e}") from e
    
    def _get_font(self, font_family: str, font_size: int, font_weight: str = "normal") -> ImageFont.ImageFont:
        ...
        if cache_key not in self.font_cache:
            try:
                ...
                logger.warning(f"Could not load font '{font_family}' with weight '{font_weight}', using default")
                try:
    ...
    def _load_font_with_weight(self, font_family: str, font_size: int, font_weight: str) -> ImageFont.ImageFont:
        ...
        font_variants = {
            ...
            "System": {
                "normal": [".SF NS Text", ".SFNS-Display", "San Francisco", "Helvetica Neue"],
                "bold": [".SF NS Text Bold", ".SFNS-Display Bold", "San Francisco Bold", "Helvetica Neue Bold"]
            }
        }
        
        # Try to find the appropriate font variant
        variants = font_variants.get(font_family, {"normal": [font_family], "bold": [font_family]})
    ...
    def _prepare_text(self, text: str, font: ImageFont.ImageFont, max_width: Optional[int], max_lines: Optional[int] = None, canvas_width: Optional[int] = None) -> list[str]:
    ...
    def _calculate_anchor_position(self, position: Tuple[int, int], text_lines: list[str], 
                                  font: ImageFont.ImageFont, line_height: int, anchor: str, 
                                  max_width: Optional[int], canvas: Image.Image, 
                                  text_config) -> Tuple[int, int]:
    ...
    def _calculate_line_x(self, base_x: int, line: str, font: ImageFont.ImageFont, 
                         alignment: str, alignment_width: int) -> int:
