...
class DeviceFrameRenderer:
    ...
    def render(self, device_frame_name: str, canvas: Image.Image, source_image: Image.Image) -> Image.Image:
        ...
        try:
            ...
            if frame_info and ('screen_bounds' in frame_info or 'x' in frame_info):
                ...
                result = self._compose_with_metadata(canvas, source_image, frame_image, frame_info)
            else:
                # Fall back to simple overlay approach
                logger.warning(f"No metadata found for {device_frame_name}, using simple overlay")
                result = self._compose_simple_overlay(canvas, frame_image)
            
            return result
            
        except Exception as e:
            raise DeviceFrameError(f"Failed to render device frame '{device_frame_name}': {e}") from e
    ...
    def _get_frame_info(self, frame_name: str) -> Optional[Dict[str, Any]]:
        ...
        if len(parts) >= 3:
            ...
            if len(model_parts) >= 2:
                ...
                if not current:
                    logger.warning(f"Variant '{variant}' not found under {device_type} {model_key}")
                    return None
    ...
    def _compose_with_metadata(self, canvas: Image.Image, source_image: Image.Image, 
                              frame_image: Image.Image, frame_info: Dict[str, Any]) -> Image.Image:
        ...
        logger.info(f"📐 Screen area: {screen_x}, {screen_y}, {screen_width}×{screen_height}")
        ...
        logger.info(f"📐 Scaling source {source_width}×{source_height} to {final_width}×{final_height} (scale: {scale:.2f})")
        
        # Resize source image to fit screen area
        source_image = source_image.resize((final_width, final_height), Image.Resampling.LANCZOS)
    ...
    def _compose_simple_overlay(self, canvas: Image.Image, frame_image: Image.Image) -> Image.Image:
    ...
    def generate_screen_mask(self, frame_name: str) -> Image.Image:
        ...
        try:
            ...
            logger.info(f"📱 Creating mask from alpha channel analysis")
            ...
            logger.info(f"📱 Generated mask using frame alpha channel (alpha=0 -> screen area)")
            
            screen_mask = mask
            logger.info(f"📱 Generated screen mask: low_alpha≤50=show, high_alpha>200=hide")
            
            logger.info(f"📱 Generated screen mask for {frame_name} using boundary detection")
            return screen_mask
            
        except Exception as e:
            logger.error(f"Failed to generate screen mask for {frame_name}: {e}")
    ...
    def generate_screen_mask_from_image(self, frame_image: Image.Image) -> Image.Image:
        ...
        try:
            ...
            logger.info(f"📱 Creating precise mask using flood fill from scaled frame ({frame_width}x{frame_height})")
            ...
            logger.info(f"📱 Flood fill completed: {len(visited)} pixels marked as outside, {(frame_width*frame_height-len(visited))} as screen area")
            
            logger.info(f"📱 Generated precise mask from scaled frame (no resize distortion)")
            
            # DEBUG: Create a tinted version to visualize the mask
            # Convert mask to RGBA and tint the masked areas
            debug_mask = Image.new('RGBA', frame_image.size, (0, 0, 0, 0))  # Transparent
            debug_pixels = debug_mask.load()
            
            for y in range(frame_height):
                for x in range(frame_width):
                    if mask_pixels[x, y] == 255:  # Screen area (white in mask)
                        debug_pixels[x, y] = (0, 255, 0, 100)  # Semi-transparent green = where content shows
                    else:  # Frame area (black in mask)
                        debug_pixels[x, y] = (255, 0, 0, 100)  # Semi-transparent red = where content is hidden
            
            # Save debug mask for inspection
            debug_path = f"/tmp/debug_mask_{frame_width}x{frame_height}.png"
            ...
            logger.error(f"Failed to generate screen mask from frame image: {e}")
            # Return a fallback mask (full white - no clipping)
            return Image.new('L', frame_image.size, 255)
    
    def apply_screen_mask(self, canvas_image: Image.Image, mask: Image.Image, asset_position: tuple, asset_size: tuple) -> Image.Image:
        ...
        try:
            ...
            canvas_mask = Image.new('L', canvas_image.size, 0)  # Start with black (hide all)
            ...
            logger.error(f"Failed to apply screen mask to canvas: {e}")
